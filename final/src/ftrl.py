...
test_header = [ 'display_id', 'ad_id', 'probability' ]

with open( test ) as infile:
    test_dict = {}
    for t, disp_id, ad_id, x, y in data(test, D):
        p = learner.predict(x)
        if disp_id not in test_dict:
            test_dict[ disp_id ] = []
        test_dict[ disp_id ].append( [ ad_id, p ] )
        if t % 1000000 == 0:
            print "Test : ", t
print(len(test_dict))
...
with open(submission, 'w') as outfile:
    ...
    for disp_id in test_dict:
        outfile.write( '%s,' % disp_id )
        test_dict[ disp_id ].sort( key = lambda x : x[ 1 ] )
        for i in range( len( test_dict[ disp_id ] ) - 1 ):
            outfile.write( '%s ' % test_dict[ disp_id ][ i ][ 0 ] )
        outfile.write( '%s\n' % test_dict[ disp_id ][ -1 ][ 0 ] )
